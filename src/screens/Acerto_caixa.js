...
import AddAcerto_caixa from './AddAcerto_caixa'
...
export default class Acerto_caixa extends Component {
    state = {
        acessoemps: [],
        usuario: [],
        visible: [],
...
//---------------------------------------------------------------------------------------
    AddAcerto_caixas = async  locData  => {
        this.setState({loading: true})
        try {
            const res = await axios.post(`${server}/acerto_caixas`, {
                acerto_caixasDataCadastro: new Date(), 
...
//--------------------------------------------------------------------------------------- 
    updateAcerto_caixas = async  acerto_caixa  => {
        const data = new Date()
        console.log(data.getHours())
        try {
            await axios.put(`${server}/acerto_caixas/atualisa`, {
                acerto_caixas_id: dados.id,
                acerto_caixasDataUpdate: new Date(),
                acerto_caixasOperador: acerto_caixa.acerto_caixasOperador,
                acerto_caixasMotivo: acerto_caixa.acerto_caixasMotivo,
                acerto_caixasDiferenca: acerto_caixa.acerto_caixasDiferenca,
                acerto_caixasAuditor: acerto_caixa.acerto_caixasAuditor,
                acerto_caixasResponsavel: acerto_caixa.acerto_caixasResponsavel,
                acerto_caixasObservacao: acerto_caixa.acerto_caixasObservacao                
            })
        ...
        if(this.state.loading){
            ...
            return (
                <View style={styles.container}>
                    <AddAcerto_caixa {...this.state} isVisible={this.state.showAdd} 
                        onSave={this.updateAcerto_caixas}
                    ...
                    <View style={styles.taksContainer}>
                        <Text style={styles.titletela}>Controle de Acerto_caixa</Text>
                        <View style={styles.linha} />
                    </View>
