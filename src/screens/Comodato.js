...
import AddComodato from './AddComodato'
...
export default class Comodato extends Component {
    state = {
        acessoemps: [],
        usuario: [],
        visible: [],
...
//---------------------------------------------------------------------------------------
    AddComodatos = async  locData  => {
        this.setState({loading: true})
        try {
            const res = await axios.post(`${server}/comodatos`, {
                comodatosDataCadastro: new Date(), 
...
//--------------------------------------------------------------------------------------- 
    updateComodatos = async  comodato  => {
        const data = new Date()
        console.log(data.getHours())
        try {
            await axios.put(`${server}/comodatos/atualisa`, {
                comodatos_id: dados.id,
                comodatosDataUpdate: new Date(),
                comodatosData: new Date(),
                comodatosHora: (data.getHours()-3)+':'+data.getMinutes(),
                comodatosFornecedor: comodato.comodatosFornecedor,
                comodatosNota: comodato.comodatosNota,
                comodatosEquipamento: comodato.comodatosEquipamento,
                comodatosQuantidade: comodato.comodatosQuantidade,
                comodatosPrevencao: comodato.comodatosPrevencao, 
                comodatosObservacao: comodato.comodatosObservacao                
            })
        ...
        if(this.state.loading){
            ...
            return (
                <View style={styles.container}>
                    <AddComodato {...this.state} isVisible={this.state.showAdd} 
                        onSave={this.updateComodatos}
                    ...
                    <View style={styles.taksContainer}>
                        <Text style={styles.titletela}>Controle de Comodato</Text>
                        <View style={styles.linha} />
                    </View>
