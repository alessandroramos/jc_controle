...
export default class Epi extends Component {
    state = {
        acessoemps: [],
        usuario: [],
        visible: [],
...
//---------------------------------------------------------------------------------------
    AddEpis = async  locData  => {
        this.setState({loading: true})
        try {
            const res = await axios.post(`${server}/epis`, {
                episDataCadastro: new Date(), 
...
//--------------------------------------------------------------------------------------- 
    updateEpis = async  epi  => {
        const data = new Date()
        console.log(data.getHours())
        try {
            await axios.put(`${server}/epis/atualisa`, {
                epis_id: dados.id,
                episDataUpdate: new Date(),

                episRetirada: epi.episRetirada,
                episDevolucao: epi.episDevolucao,
                episQuantidade: epi.episQuantidade,
                episUnidade: epi.episUnidade,
                episDescEquipamneto: epi.episDescEquipamneto,
                episNumCA: epi.episNumCA,
                episColaborador: epi.episColaborador,
                episResponsavel: epi.episResponsavel,
                episObservacao: epi.episObservacao                
            })
        ...
        if(this.state.loading){
            ...
            return (
                <View style={styles.container}>
                    <AddEpi {...this.state} isVisible={this.state.showAdd} 
                        onSave={this.updateEpis}
                    ...
                    <View style={styles.taksContainer}>
                        <Text style={styles.titletela}>Controle de Epi</Text>
                        <View style={styles.linha} />
                    </View>
