...
import AddManifesto from './AddManifesto'
...
//---------------------------------------------------------------------------------------
    AddManifestos = async  locData  => {
        this.setState({loading: true})
        try {
            const res = await axios.post(`${server}/manifestos`, {
                manifestosDataCadastro: new Date(), 
...
//--------------------------------------------------------------------------------------- 
    updateManifestos = async  manifesto  => {
        const data = new Date()
        console.log(data.getHours())
        try {
            await axios.put(`${server}/manifestos/atualisa`, {
                manifestos_id: dados.id,
                manifestosDataUpdate: new Date(),
                manifestosHora: (data.getHours()-3)+':'+data.getMinutes(),
                
                manifestosInicialFinal: manifesto.manifestosInicialFinal,
                manifestosOrigem: manifesto.manifestosOrigem,
                manifestosDestino: manifesto.manifestosDestino,
                manifestosLacre: manifesto.manifestosLacre,
                manifestosFiscal: manifesto.manifestosFiscal, 
                manifestosObservacao: manifesto.manifestosObservacao                
            })
        ...
        if(this.state.loading){
            ...
            return (
                <View style={styles.container}>
                    <AddManifesto {...this.state} isVisible={this.state.showAdd} 
                        onSave={this.updateManifestos}
                    ...
                    <View style={styles.taksContainer}>
                        <Text style={styles.titletela}>Controle de Manifesto</Text>
                        <View style={styles.linha} />
                    </View>
