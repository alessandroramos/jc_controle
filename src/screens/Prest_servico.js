...
import AddPrest_servico from './AddPrest_servico'
...
export default class Prest_servico extends Component {
    state = {
        acessoemps: [],
        usuario: [],
        visible: [],
...
//---------------------------------------------------------------------------------------
    AddPrest_servicos = async  locData  => {
...
//--------------------------------------------------------------------------------------- 
    updatePrest_servicos = async  prest_servico  => {
        const data = new Date()
        console.log(data.getHours())
        try {
            await axios.put(`${server}/prest_servicos/atualisa`, {
                prest_servicos_id: dados.id,
                prest_servicosDataUpdate: new Date(),

                prest_servicosHora: (data.getHours()-3)+':'+data.getMinutes(),
                prest_servicosEntSai: prest_servico.prest_servicosEntSai,
                prest_servicosData: new Date(),
                prest_servicosNomePres: prest_servico.prest_servicosNomePres,
                prest_servicosDocumento: prest_servico.prest_servicosDocumento,
                prest_servicosEmpresa: prest_servico.prest_servicosEmpresa, 
                prest_servicosSolicitante: prest_servico.prest_servicosSolicitante,
                prest_servicosResponsavel: prest_servico.prest_servicosResponsavel, 
                prest_servicosObservacao: prest_servico.prest_servicosObservacao,
                })
...
//---------------------------------------------------------------------------------------    
    abriAdd = () =>{
        Geolocation.getCurrentPosition((locData)=>{
            this.AddPrest_servicos(locData)
        }, (erro)=>{
        ...
        if(this.state.loading){
            ...
            return (
                <View style={styles.container}>
                    <AddPrest_servico {...this.state} isVisible={this.state.showAdd} 
                        onSave={this.updatePrest_servicos}
                    ...
                    <View style={styles.taksContainer}>
                        <Text style={styles.titletela}>Controle de Prestação de  Serviço</Text>
                        <View style={styles.linha} />
                    </View>
