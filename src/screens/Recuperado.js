...
import AddRecuperado from './AddRecuperado'
...
export default class Recuperado extends Component {
    state = {
        acessoemps: [],
        usuario: [],
        visible: [],
...
//---------------------------------------------------------------------------------------
    AddRecuperados = async  locData  => {
        this.setState({loading: true})
        try {
            const res = await axios.post(`${server}/recuperados`, {
                recuperadosDataCadastro: new Date(), 
...
//--------------------------------------------------------------------------------------- 
    updateRecuperados = async  recuperado  => {
        const data = new Date()
        console.log(data.getHours())
        try {
            await axios.put(`${server}/recuperados/atualisa`, {
                recuperados_id: dados.id,
                recuperadosDataUpdate: new Date(),

                recuperadosProduto:recuperado.recuperadosProduto,
                recuperadosQuantidade:recuperado.recuperadosQuantidade,
                recuperadosValor:recuperado.recuperadosValor,
                recuperadosPrevencao: recuperado.recuperadosPrevencao, 
                recuperadosObservacao: recuperado.recuperadosObservacao                
            })
        ...
        if(this.state.loading){
            ...
            return (
                <View style={styles.container}>
                    <AddRecuperado {...this.state} isVisible={this.state.showAdd} 
                        onSave={this.updateRecuperados}
                    ...
                    <View style={styles.taksContainer}>
                        <Text style={styles.titletela}>Controle de Recuperado</Text>
                        <View style={styles.linha} />
                    </View>
