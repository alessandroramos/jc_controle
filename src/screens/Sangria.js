...
import AddSangria from './AddSangria'
...
export default class Sangria extends Component {
    state = {
        acessoemps: [],
        usuario: [],
        visible: [],
...
//---------------------------------------------------------------------------------------
    AddSangrias = async  locData  => {
        this.setState({loading: true})
        try {
            const res = await axios.post(`${server}/sangrias`, {
                sangriasDataCadastro: new Date(), 
...
//--------------------------------------------------------------------------------------- 
    updateSangrias = async  sangria  => {
        const data = new Date()
        console.log(data.getHours())
        try {
            await axios.put(`${server}/sangrias/atualisa`, {
                sangrias_id: dados.id,
                sangriasDataUpdate: new Date(),

                sangriasEnvelope: sangria.sangriasEnvelope,
                sangriasValor: sangria.sangriasValor,
                sangriasOperador: sangria.sangriasOperador,
                sangriasPrevencao: sangria.sangriasPrevencao, 
                sangriasObservacao: sangria.sangriasObservacao                
            })
        ...
        if(this.state.loading){
            ...
            return (
                <View style={styles.container}>
                    <AddSangria {...this.state} isVisible={this.state.showAdd} 
                        onSave={this.updateSangrias}
                    ...
                    <View style={styles.taksContainer}>
                        <Text style={styles.titletela}>Controle de Sangria</Text>
                        <View style={styles.linha} />
                    </View>
