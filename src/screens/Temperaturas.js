...
//---------------------------------------------------------------------------------------
    AddTemperaturas = async  locData  => {
        this.setState({loading: true})
        try {
            const res = await axios.post(`${server}/temperaturas`, {
                temperaturasDataCadastro: new Date(),
...
//--------------------------------------------------------------------------------------- 
    updateTemperaturas = async  temperatura  => {
        const data = new Date()
        console.log(data.getHours())
        try {
            await axios.put(`${server}/temperaturas/atualisa`, {
                temperaturas_id: dados.id,
                temperaturasDataUpdate: new Date(),

                temperaturasSetor: temperatura.temperaturasSetor,
                temperaturasEquipamento: temperatura.temperaturasEquipamento,
                temperaturasTempIdeal: temperatura.temperaturasTempIdeal,
                temperaturasHora: (data.getHours()-3)+':'+data.getMinutes(),
                temperaturasTemperatura: temperatura.temperaturasTemperatura,
                temperaturasResponsavel: temperatura.temperaturasResponsavel,
                temperaturasPrevencao: temperatura.temperaturasPrevencao, 
                temperaturasObservacao: temperatura.temperaturasObservacao                
            })
        ...
        if(this.state.loading){
            ...
            return (
                <View style={styles.container}>
                    <AddTemperatura {...this.state} isVisible={this.state.showAdd} 
                        onSave={this.updateTemperaturas}
                    ...
                    <View style={styles.taksContainer}>
                        <Text style={styles.titletela}>Controle de Temperatura</Text>
                        <View style={styles.linha} />
                    </View>
