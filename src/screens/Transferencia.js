...
import AddTransferencia from './AddTransferencia'
...
//---------------------------------------------------------------------------------------
    AddTransferencias = async  locData  => {
        this.setState({loading: true})
        try {
            const res = await axios.post(`${server}/transferencias`, {
                transferenciasDataCadastro: new Date(), 
...
//--------------------------------------------------------------------------------------- 
    updateTransferencias = async  transferencia  => {
        const data = new Date()
        console.log(data.getHours())
        try {
            await axios.put(`${server}/transferencias/atualisa`, {
                transferencias_id: dados.id,
                transferenciasDataUpdate: new Date(),
                transferenciasData: new Date(),
                transferenciasHora: (data.getHours()-3)+':'+data.getMinutes(),
                transferenciasNota: transferencia.transferenciasNota,
                transferenciasMotorista: transferencia.transferenciasMotorista,
                transferenciasSeparador: transferencia.transferenciasSeparador,
                transferenciasConferente: transferencia.transferenciasConferente,                
                transferenciasPrevencao: transferencia.transferenciasPrevencao, 
                transferenciasObservacao: transferencia.transferenciasObservacao                
            })
...
//---------------------------------------------------------------------------------------    
    abriAdd = () =>{
        Geolocation.getCurrentPosition((locData)=>{
            this.AddTransferencias(locData)
        }, (erro)=>{
        ...
        if(this.state.loading){
            ...
            return (
                <View style={styles.container}>
                    <AddTransferencia {...this.state} isVisible={this.state.showAdd} 
                        onSave={this.updateTransferencias}
                    ...
                    <View style={styles.taksContainer}>
                        <Text style={styles.titletela}>Controle de Transferencia</Text>
                        <View style={styles.linha} />
                    </View>
